    ...
    public async Task<(IReadOnlyList<UnpaidStudentRow> Items, int TotalCount)>
            ...
            DateTime throughMonthEnd,
                int page, int pageSize)
        {
        // Active (non-deleted) students only â€” the global !IsDeleted filter applies. A student in
        ...
        // Arrears are judged ONLY through the cutoff month (CLAUDE.md Â§7.4). Periods are
        // pre-generated to the session end, so an all-time scan reports every FUTURE month as
        // owed â€” the defect this replaces. Orphaned periods (TeacherStudentId nulled on student
        ...
        // Scoped by the PERIOD's session/group â€” NOT the student's current assignment. A student
        ...
        // always a contiguous tail â€” the unpaid count IS the consecutive count (BR-PAY-006).
        var grouped = periods
            .GroupBy(p => p.TeacherStudentId!.Value)
            .Select(g => new
            {
        ...
        var pageRows = await grouped
            ...
            // Deterministic tiebreak â€” ties are common, and Skip/Take must be stable across pages.
        ...
        // is a historical fact, so it is still read from the counter â€” unlike the arrears totals,
        ...
        // stays in the Application layer â€” the repo returns dates, not display strings.
