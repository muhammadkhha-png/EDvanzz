...
**CI migration delivery — root cause of the 2026-07-15/16 attendance outage (deploy.yml `Apply EF migrations`).** `azure/sql-action@v2` runs the multi-batch idempotent `migrate.sql` (one `BEGIN TRAN…COMMIT` per migration) via go-sqlcmd **without `-b`**, so when a migration's batch errors, its own transaction rolls back (migration NOT recorded) but the runner **continues to the next migration and still exits 0**. Net: a broken migration is silently skipped while the migrations after it apply and the deploy ships — code reaches prod without its schema. This is why BUG-10 shipped, and it has ALSO been silently skipping the pre-existing **`20260708193718` / `20260708220307` `IX_TeacherStudents_ParentPhoneNumber` UNIQUE-index migrations since 2026-07-08** (they fail on a *soft-deleted* duplicate parent phone `01274561635`; the index does not exist on prod — a BUG-9-lineage tangle; note a parent with several children legitimately shares a phone, so that index may be wrong-by-design). **The correct fix is `arguments: '-b'` on the sql-action step (abort + non-zero exit on any SQL error, failing the job BEFORE deploy) — but it CANNOT be enabled until that parent-phone backlog is cleaned, or every deploy will abort there.** Deferred pending a decision on the parent-phone uniqueness rule; do not enable `-b` before resolving it.
