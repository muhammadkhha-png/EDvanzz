...
| **Settings → "Collect Books & fees with attendance"** toggle (added 2026-09-14) | (same PUT) | `showExtrasOnAttendanceScreen: bool` — **NULLABLE server-side on the UPDATE DTO: omitted/null = unchanged**, so an older build that never sends it cannot reset a teacher's choice on an unrelated whole-object save (the `examAttachmentReleaseDelayHours` / `billingStartDate` precedent). The GET returns it as `bool?`, read as `?? true`. This is the GLOBAL half of a two-level switch — each item also carries its own `collectDuringAttendance`, so a رحلة can stay off the classroom door while a مذكرة is chased there | — |
| **Settings → "Student sees Books & fees"** toggle | (same PUT) | `studentVisibilityExtras: bool?` — **NULLABLE on the UPDATE (omitted = unchanged)** even though the column is not, so an older build cannot turn family-facing exposure on or off by omission; the GET returns a plain `bool`. **Defaults FALSE**, unlike every other `studentVisibility*` flag: no family may receive a debt notice until the teacher deliberately turns it on. This is the app's FIRST student-visibility switch — the other four `studentVisibility*` flags are parsed into `TeacherConfiguration` but have no cubit setter and no view, i.e. they are server-side defaults only. It lives in its own one-switch group in General settings for exactly that reason: putting it in the group above would imply those four are editable too | — |
...
| **Parent visibility: Books & fees** (added 2026-09-14) | (same PUT) | `parentVisibilityExtras: bool?` — **NULLABLE on the UPDATE (omitted = unchanged)**, same rule as its student twin; the GET returns a plain `bool`. **Defaults FALSE**, same conservative stance. Lives in the Parent-Portal settings section beside `parentVisibilityPayment`, NOT in general Settings, because that is where a teacher already manages what parents see. Read through the SHARED gate (`PaymentViewerType.Parent`), so the student and parent answers cannot drift | — |
