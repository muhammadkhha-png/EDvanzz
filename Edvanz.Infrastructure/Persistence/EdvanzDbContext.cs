...
public class EdvanzDbContext(DbContextOptions<EdvanzDbContext> options) : DbContext(options)
{
    ...
    // ─── Attendance module tables (Module 3) ───
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        #region SessionOccurrence (REQ-ATT-001/002/005)
        modelBuilder.Entity<SessionOccurrence>(entity =>
        {
            entity.ToTable("SessionOccurrences");

            // OccurrenceDate: date-only column
            entity.Property(o => o.OccurrenceDate)
                .HasColumnType("date")
                .IsRequired();

            // OccurrenceIndex: sequential index within the session's schedule
            entity.Property(o => o.OccurrenceIndex).IsRequired();

            // Session FK: cascade delete when session is hard-deleted (REQ-SES-041)
            ...
            // Primary lookup: one occurrence per session per date (UNIQUE)
            entity.HasIndex(o => new { o.SessionId, o.OccurrenceDate })
                .IsUnique()
                .HasDatabaseName("IX_SessionOccurrences_SessionId_OccurrenceDate");

            // Sequential navigation: "previous occurrence" lookup
            entity.HasIndex(o => new { o.SessionId, o.OccurrenceIndex })
                .HasDatabaseName("IX_SessionOccurrences_SessionId_OccurrenceIndex");

            // Daily dashboard: "which sessions occur today?"
            entity.HasIndex(o => o.OccurrenceDate)
                .HasDatabaseName("IX_SessionOccurrences_OccurrenceDate");
        });
        #endregion
        ...
        modelBuilder.Entity<StudentSessionAssignment>(entity =>
        {
            ...
            // SessionNameSnapshot: preserved session name for history display
            entity.Property(a => a.SessionNameSnapshot)
                .HasMaxLength(200)
                .IsRequired();

            // AssignedAt: timestamp with full precision
            entity.Property(a => a.AssignedAt).IsRequired();

            // Teacher FK: cascade delete
            entity.HasOne(a => a.Teacher)
                .WithMany()
                .HasForeignKey(a => a.TeacherId)
                .OnDelete(DeleteBehavior.Cascade);

            // TeacherStudent FK: cascade delete
            entity.HasOne(a => a.TeacherStudent)
                .WithMany()
                .HasForeignKey(a => a.TeacherStudentId)
                .OnDelete(DeleteBehavior.Cascade);

            // Session FK: SET NULL when session is deleted (BR-ATT-005)
            ...
            // Enforce at most one active assignment per student
            // Filtered unique index: only applies where IsActive = true
            entity.HasIndex(a => new { a.TeacherStudentId, a.IsActive })
                .HasFilter("[IsActive] = 1")
                .IsUnique()
                .HasDatabaseName("IX_StudentSessionAssignments_StudentId_Active");

            // "Which students are assigned to this session?" lookup
            entity.HasIndex(a => a.SessionId)
                .HasDatabaseName("IX_StudentSessionAssignments_SessionId");

            // Student timeline: all assignment periods for a student
            entity.HasIndex(a => new { a.TeacherId, a.TeacherStudentId })
                .HasDatabaseName("IX_StudentSessionAssignments_TeacherId_StudentId");
        });
        #endregion

        #region AttendanceRecord (REQ-ATT-006/007/017/025)
        modelBuilder.Entity<AttendanceRecord>(entity =>
        {
            entity.ToTable("AttendanceRecords");

            // OccurrenceDate: denormalized, survives session deletion (BR-ATT-005)
            entity.Property(ar => ar.OccurrenceDate)
                .HasColumnType("date")
                .IsRequired();

            // RecordedAt: timestamp with minute precision (REQ-ATT-NFR-004)
            entity.Property(ar => ar.RecordedAt).IsRequired();

            // Teacher FK: cascade delete
            entity.HasOne(ar => ar.Teacher)
                .WithMany()
                .HasForeignKey(ar => ar.TeacherId)
                .OnDelete(DeleteBehavior.Cascade);

            // TeacherStudent FK: RESTRICT — attendance survives student soft-delete
            entity.HasOne(ar => ar.TeacherStudent)
                .WithMany()
                .HasForeignKey(ar => ar.TeacherStudentId)
                .OnDelete(DeleteBehavior.Restrict);

            // StudentSessionAssignment FK: cascade delete
            entity.HasOne(ar => ar.StudentSessionAssignment)
                .WithMany(a => a.AttendanceRecords)
                .HasForeignKey(ar => ar.StudentSessionAssignmentId)
                .OnDelete(DeleteBehavior.Cascade);

            // SessionOccurrence FK: SET NULL (BR-ATT-005 — survives session deletion)
            entity.HasOne(ar => ar.SessionOccurrence)
                .WithMany()
                .HasForeignKey(ar => ar.SessionOccurrenceId)
                .OnDelete(DeleteBehavior.SetNull);

            // AttendedSessionOccurrence FK: SET NULL (cross-session reference)
            entity.HasOne(ar => ar.AttendedSessionOccurrence)
                .WithMany()
                .HasForeignKey(ar => ar.AttendedSessionOccurrenceId)
                .OnDelete(DeleteBehavior.SetNull);

            // RecordedByUser FK: SET NULL when user is deleted
            entity.HasOne(ar => ar.RecordedByUser)
                .WithMany()
                .HasForeignKey(ar => ar.RecordedByUserId)
                .OnDelete(DeleteBehavior.SetNull);

            // ── INDEXES ──

            // BR-ATT-002: One attendance record per student per occurrence (UNIQUE)
            // This is the PRIMARY performance and correctness index.
            entity.HasIndex(ar => new { ar.SessionOccurrenceId, ar.TeacherStudentId })
                .IsUnique()
                .HasFilter("[SessionOccurrenceId] IS NOT NULL")
                .HasDatabaseName("IX_AttendanceRecords_OccurrenceId_StudentId");

            // Reverse lookup: student's attendance history
            entity.HasIndex(ar => new { ar.TeacherStudentId, ar.OccurrenceDate })
                .HasDatabaseName("IX_AttendanceRecords_StudentId_OccurrenceDate");

            // Absence overview queries filtered by status
            entity.HasIndex(ar => new { ar.TeacherId, ar.Status })
                .HasDatabaseName("IX_AttendanceRecords_TeacherId_Status");

            // Assignment period history
            entity.HasIndex(ar => ar.StudentSessionAssignmentId)
                .HasDatabaseName("IX_AttendanceRecords_AssignmentId");

            // Cross-session attendance lookups (filtered: non-null only)
            entity.HasIndex(ar => ar.AttendedSessionOccurrenceId)
                .HasFilter("[AttendedSessionOccurrenceId] IS NOT NULL")
                .HasDatabaseName("IX_AttendanceRecords_AttendedOccurrenceId");
        });
        #endregion
        ...
        modelBuilder.Entity<AttendanceEditLog>(entity =>
        {
            ...
            // EditedAt: timestamp for audit trail
            entity.Property(l => l.EditedAt).IsRequired();

            // EditReason: optional free text
            entity.Property(l => l.EditReason)
                .HasMaxLength(500);

            // AttendanceRecord FK: cascade delete
            entity.HasOne(l => l.AttendanceRecord)
                .WithMany(ar => ar.EditLogs)
                .HasForeignKey(l => l.AttendanceRecordId)
                .OnDelete(DeleteBehavior.Cascade);

            // EditedByUser FK: SET NULL
            entity.HasOne(l => l.EditedByUser)
                .WithMany()
                .HasForeignKey(l => l.EditedByUserId)
                .OnDelete(DeleteBehavior.SetNull);

            // ── INDEXES ──

            // Edit history for a specific record
            entity.HasIndex(l => l.AttendanceRecordId)
                .HasDatabaseName("IX_AttendanceEditLogs_RecordId");
        });
        #endregion

        #region StudentAbsenceCounter (REQ-ATT-021/029/030/031)
        modelBuilder.Entity<StudentAbsenceCounter>(entity =>
        {
            entity.ToTable("StudentAbsenceCounters");

            // LastAttendanceDate: date-only
            entity.Property(c => c.LastAttendanceDate)
                .HasColumnType("date");

            // LastAttendanceSessionName: session name for alert context
            entity.Property(c => c.LastAttendanceSessionName)
                .HasMaxLength(200);
            ...
            // TeacherStudent FK: cascade delete — one counter per student
            ...
            // One counter per student (UNIQUE)
            entity.HasIndex(c => c.TeacherStudentId)
                .IsUnique()
                .HasDatabaseName("IX_StudentAbsenceCounters_StudentId");

            // Absence Overview sorted by consecutive absences (REQ-ATT-067)
            entity.HasIndex(c => new { c.TeacherId, c.ConsecutiveAbsences })
                .HasDatabaseName("IX_StudentAbsenceCounters_TeacherId_Consecutive");
        });
        #endregion
